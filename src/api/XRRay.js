...
export default class XRRay {
  /**
   * @param {?XRVector3} origin
   * @param {?XRVector3} direction
   * @param {Float32Array} transformMatrix
   */
  constructor(origin=new DOMPointReadOnly(0, 0, 0, 1),
              direction=new DOMPointReadOnly(0, 0, -1, 0),
              transformMatrix=new Float32Array(16)) {

    if (!(origin instanceof DOMPointReadOnly)) {
      throw new Error('origin must be a DOMPointReadOnly');
    }
    if (!(direction instanceof DOMPointReadOnly)) {
